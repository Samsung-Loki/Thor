...
public class RawWrite : ICommand {
    public FailInfo RunCommand(State state, List<string> args) {
        ...
        try {
            switch (args[0].ToLower()) {
                case "string": {
                    state.Handler.BulkWrite(Encoding.ASCII.GetBytes(args[1]));
                    break;
                }
                case "int": {
                ...
                default:
                    return new FailInfo("Invalid option choice, should be [[string/int/bytes]]", 1);
            }
        } catch (Exception e) {
    ...
    public string GetDescription()
        => "write [[string/int/bytes]] <content> - Send a packet of specified type";
}